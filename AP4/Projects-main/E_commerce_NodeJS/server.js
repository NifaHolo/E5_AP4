...
//test insc
app.post('/JSON_connexion', (request, response) => {
...
//get concernant le shop
app.get('/JSON_Produits', (request, response) => {
...
console.log(request.body)
    ...
    //  Si jamais table est set alors on elete la table
    if(request.body.table !== undefined){

        Shop.deleteAll(request.body.table, function(result){
                console.log(request.body.table+' tout rimé !')
...
//  Si il n'y a pas de libellé de fond alors :
   else {
        Shop.getLastId(function(result){
